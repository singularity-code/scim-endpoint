...
public class TokenController extends Controller {
  ...
  public ResponseEntity<?> tokenInfo(
      @RequestBody MultiValueMap<String, Object> entity, HttpServletRequest request) {

    long start = System.currentTimeMillis();

    logger.info("tokeninfo");

    if (entity.containsKey("grant_type")
        && entity.get("grant_type").get(0).equals("client_credentials")) {
      ...
      return showError(
          HttpStatus.FORBIDDEN.value(), "grant type is not of type client_credentials");
    }
  }

  private String extractCredentials(
      MultiValueMap<String, Object> entity, String authorizationHeader) {
    String credential = null;
    if (entity.containsKey(Constants.CLIENT_ID) && entity.containsKey(Constants.CLIENT_SECRET)) {
      // first fetch it from the payload
      credential =
          entity.get(Constants.CLIENT_ID).get(0).toString()
              + StringUtils.COLON
