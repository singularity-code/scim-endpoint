...
public class MongoStorage implements Storage {
  ...
  public Map<String, Object> get(String id, String version) {
    Document query =
        new Document(oid, new BsonBinary(UUID.fromString(id)))
  ...
  private FindIterable<Document> find(
      int start, int count, List<String> includeAttributes, Document query) {
  ...
  public void initialize(String type) {
    ...
    try {
      col.createIndex(
          Indexes.descending(userName), new IndexOptions().background(true).unique(true));
    } catch (Exception e) {
