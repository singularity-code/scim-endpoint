...
public class PropertyFileAuthenticationFilter implements Filter {
  ...
  private final List<String> PUBLIC_ENDPOINTS =
      Arrays.asList(new String[] {"/scim/v2/token", "/scim/v2/Me"});
  private final String serverDescription =
      PropertyFileAuthenticationFilter.class.getPackage().getImplementationTitle()
          + StringUtils.SPACE
          + PropertyFileAuthenticationFilter.class.getPackage().getImplementationVersion();
      ...
      if (header != null) {
        ...
        if (auth.length == 2) {
          ...
          if (auth[0].equalsIgnoreCase(Constants.BASIC)) {
            ...
            if (authenticationUtils.getBasicAuthUsers() != null
                && authenticationUtils.getBasicAuthUsers().containsKey(credential)) {
              // check roles
              filtered =
                  checkRole(
                      request,
                      ...
                      credential,
                      method,
                      authenticationUtils.getBasicAuthUsers());
            }
            ...
            if (tokenUtils.isValid(token)) {
              ...
              filtered =
                  checkRole(
                      ...
                      token,
                      method,
                      authenticationUtils.getBearerAuthUsers());
            }
          }
        }
      }
